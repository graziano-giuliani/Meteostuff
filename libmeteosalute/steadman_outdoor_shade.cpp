...
float *Meteosalute::steadman_outdoor_shade(float *t, float *rh, float *wind)
{
  if (vector_size <= 0) return 0;
  float *steadman_outdoor_shade = new float[vector_size];

  for (int i = 0; i < vector_size; i ++)
  {
    steadman_outdoor_shade[i] = 999.9;
    ...
    else
    {
      ...
      steadman_outdoor_shade[i] = t[i]+(0.33*e)-(0.7*wind[i])-4.0;
    }
  }
...
int main(int argc, char *argv[])
{
  ...
  float *we = m.steadman_outdoor_shade(t, rh, wind);
