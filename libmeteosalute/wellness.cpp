...
float *Meteosalute::wellness(float *t, float *rh, float *wind)
{
  if (vector_size <= 0)
    return 0;
  float *wellness = new float[vector_size];
  ...
  for (int i = 0; i < vector_size; i ++)
  {
    ...
    if (rh[i] > 100.1|| rh[i] < 0.0)
      ...
      wellness[i] = windchill_class(t[i], wind[i]);
    else if (t[i] <= 15.0&& wind[i] < 1.3)
      wellness[i] = sharlau_class(t[i], rh[i]);
    else
      wellness[i] = hi_class(t[i], rh[i], wind[i]);
  }
