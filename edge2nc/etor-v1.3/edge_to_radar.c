...
** $Source: /nfs/trmm/src/CVS/etor/edge_to_radar.c,v $
**
** MODIFICATION RECORD
**
...
Radar *RSL_EDGE_to_radar(char *EDGE_filename)
{
	...
	int i,j,k,index;
	Sweep *sweep_u,*sweep_z,*sweep_v,*sweep_w,*sweep_d;
...
/** Allocating memory for radar structure **/
	RSL_rad = RSL_new_radar(NEEDED_VOLS);
	if (RSL_rad == NULL)
	{
	...
	meansr = 0.0;
	if (radar_verbose_flag) printf(" start: num_sweeps: %d  meansr: %f\n",num_sweeps,meansr);
	if (num_sweeps > 1)
	{
	...
	if (radar_verbose_flag) printf("end: num_sweeps: %d  meansr: %f\n",num_sweeps,meansr);

        time_t xtime = EDGE_vol->date;
	voltime = gmtime(&xtime);
	if (num_sweeps > MAX_SWEEPS) num_sweeps = MAX_SWEEPS;
	sprintf(state,"NA");
	if (radar_verbose_flag) printf("num_sweeps: %d   voltime: %d\n",num_sweeps,voltime);
/*  Now fill the Radar header  */
	...
	if (radar_verbose_flag) printf(" %2d:%2d:%2d  %2d/%2d/%4d\n",RSL_rad->h.hour,
		RSL_rad->h.minute,RSL_rad->h.sec,RSL_rad->h.month,RSL_rad->h.day,RSL_rad->h.year);
	...
	if(radar_verbose_flag) printf("lat: %f  long: %f\n",lat,lon);
